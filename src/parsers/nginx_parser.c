...
/**
 * @file nginx_parser.c
 * @brief Nginx configuration file parser (crypto-focused)
 *
 * Parses Nginx-style configs focusing on SSL/TLS directives.
 * Handles nested blocks with braces { }.
 * Simplified: no includes, no conditionals, only ssl_* and proxy_ssl_* directives.
 */
 ...
 * Only parse ssl_* and proxy_ssl_* directives
 */
static bool is_crypto_directive(const char* key) {
    if (!key) return false;

    return (strncmp(key, "ssl_", 4) == 0 ||
            strncmp(key, "proxy_ssl_", 10) == 0);
}
...
int nginx_parser_parse(
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        // Check for block open: server {
        char* brace_open = strchr(line, '{');
        if (brace_open) {
            *brace_open = '\0';
            trim_inplace(line);
            if (line[0] != '\0') {
                context_stack_push(stack, line);
            }
            continue;
        }
        ...
        // Parse directive ending with semicolon
...
/**
 * Register Nginx parser
 */
void nginx_parser_register(void) {
